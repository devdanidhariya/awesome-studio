...
class awesome2_register{
	...
	function custom_metabox(){
		...
		$args = $this->args();
		$id = $args['id'];
		if(!in_array('user',$args['object_types'])){
			$custom_meta_boxes=&aw2_library::get_array_ref('custom_meta_boxes');		
		}
		else{
			$custom_meta_boxes=&aw2_library::get_array_ref('custom_user_metaboxes');
		}
	...
	function image_size(){
		...
		$args = $this->args();
		$image_sizes[$name] = $args;
		
		aw2_library::set('image_sizes',$image_sizes );		
