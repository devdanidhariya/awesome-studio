...
aw2_library::add_shortcode('aw2','query', 'awesome2_query');

function awesome2_query($atts,$content=null,$shortcode){
	...
	$return_value=aw2_library::get('db.' . $main,$atts,$content);
...
class awesome2_query{
	...
	function set_post_terms(){
		...
		if($this->att('slugs')){
			...
			foreach ($pieces as $value){
				//$one_term=aw2_optimised::vip_get_term_by( 'slug', $value, $this->att('taxonomy') );
	...
	function get_term_by(){
		return aw2_optimised::vip_get_term_by( $this->att('field'),$this->att('value'),$this->att('taxonomy'),$this->att('output',ARRAY_A),$this->att('filter') );;	
	}
	...
	function delete_revisions(){
		$this->content="delete FROM [aw2.get wpdb.posts] where post_type='revision' and post_parent=" . $this->att['post_id'];
