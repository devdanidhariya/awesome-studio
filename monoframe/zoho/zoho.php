...
class awsZohoConfig { 
    ...
    public function makeZohoTokenFolder(){
        echo $folder=dirname(getcwd(), 1) .'/' .ZOHO_TOKEN_FOLDER_NAME;
    ...
    public function getSetZohoConfig(){
		$awesome_site_settings = get_option('awesome_site');

	    $zoho_config = array();
        foreach ($GLOBALS['aws_zoho_config_meta_keys'] as $meta_key) {
            $zoho_config[$meta_key] = trim($awesome_site_settings[$meta_key]);
        }
