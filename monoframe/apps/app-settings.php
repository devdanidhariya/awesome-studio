<?php
add_action('admin_enqueue_scripts', 'Awesome_App_Settings::load_scripts');
class Awesome_App_Settings {
	...
	static function load_scripts($hook) {
		...
		if($hook!='toplevel_page_awesome_site' && strpos($hook,'app_page_awesome-app-settings') === FALSE )
			return;
    ...
    public static function display_setting($app_slug, $option_tabs, $tab_url,$panel) {
    	...
    	$tab_forms = array();     	   	
        self::add_css();
		$count_tabs = count($option_tabs);
		
		if(empty($panel) && $count_tabs){
			$option_tab = reset($option_tabs);
			$panel = $option_tab['id'];
		}
		
		if(!$count_tabs){
			echo '<div class="bhoechie-tab-content active"> <p> No Settings for this App.</p></div> ';
			return;
		}
		?>
				...
				<div class="list-group">
					<?php 
					  ...
					  foreach ($option_tabs as $option_tab) :
						...
						if ( $tab_slug == $panel || $count_tabs == 1) {
