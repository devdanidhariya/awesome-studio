...
class Monoframe_Site_Settings {
    ...
    public function admin_page_display() {
		...
		if(!$count_tabs){
			echo '<div class="wrap cmb_options_page"> <p> No setting panel for this site, if you need, you can install them from <a href="'.admin_url('admin.php?page=awesome-studio&active_object=triggers#settings-panel').'">Awesome Catalogue</a>.</p></div> ';
			return;
		}
        ...
        <div class="wrap cmb_options_page <?php echo $this->key; ?>">        	
            ...
            <h2 class="nav-tab-wrapper">
				...
				foreach ($option_tabs as $option_tab) :
					...
					if(!isset($_GET['tab']) && $tab_slug ==$default_tab['id']){
						$nav_class .= ' nav-tab-active'; //add active class to current tab
            			$tab_forms[] = $option_tab; //add current tab to forms to be rendered
