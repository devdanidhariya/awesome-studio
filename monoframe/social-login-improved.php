...
Class awesome_social{
	...
	static function login( $provider_name ){
		try {
			...
			$hd ='';
			if(!empty($monomyth_options['opt-google-hd'])){
				$hd=$monomyth_options['opt-google-hd'];
			}
			...
			$config = array(
				...
				"providers" => array (
					...
					"Google" => array (
						...
						"approval_prompt" => "",     // optional value force
						"hd" => $hd,     // optional value force
					),
		...
		if(!empty($_GET['slug'])){
			...
			echo aw2_library::run_module($_GET['slug']);
		} else {
