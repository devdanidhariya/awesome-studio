...
Allows you to enqueue <code>.less</code> files and have them automatically compiled whenever a change is detected.
Author:       Robert O'Rourke
Contributors: Franz Josef Kaiser, Tom Willmot, Rarst
Version:      2.1
URI:   http://interconnectit.com
License:      MIT
*/
...
if ( ! class_exists( 'lessc' ) ) {
	...
	if ( file_exists( __DIR__.'/lessphp/lessc.inc.php' ) ) {
		// load LESS parser
...
class wp_less {
	...
	public function parse_stylesheet( $src, $handle ) {
		...
		list( $less_path2, $less_path ) = explode( 'wp-content', $less_path );

$less_path=WP_CONTENT_DIR.$less_path;		
		...
		try {
			...
			if ( empty( $cache ) || empty( $cache[ 'less' ][ 'updated' ] ) || $less_cache[ 'updated' ] > $cache[ 'less' ][ 'updated' ] || $this->vars !== $cache[ 'vars' ] ) {
				...
				$minify_css = preg_replace( '!/\*[^*]*\*+([^/][^*]*\*+)*/!', '', $less_cache[ 'compiled' ] );
				$minify_css = str_replace( array("\r\n", "\r", "\n", "\t", '  ', '    ', '    '), '', $minify_css );
				//file_put_contents( $css_path, $less_cache[ 'compiled' ] );
				file_put_contents( $css_path, $minify_css );
			}
		} catch ( exception $ex ) {
